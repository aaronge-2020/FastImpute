...
# Directory where the original reference VCF files are stored
reference_dir="Data/Beagle_reference_minus_test"

# Directory where the cleaned reference VCF files will be stored
cleaned_reference_dir="Data/Beagle_reference_cleaned"
mkdir -p "$cleaned_reference_dir"
...
chromosomes=(8 12 14 17)

# Function to clean reference VCFs by removing duplicate markers using bcftools norm
clean_reference_vcfs() {
  for chr in "${chromosomes[@]}"; do
    echo "Cleaning reference VCF for chromosome: $chr"

    input_vcf="${reference_dir}/ALL.chr${chr}.phase3_shapeit2_mvncall_integrated_v5b.20130502.genotypes_reference_filtered.vcf.gz"
    output_vcf="${cleaned_reference_dir}/ALL.chr${chr}.cleaned.vcf.gz"

    # Normalize and clean the VCF, removing duplicates
    bcftools norm -d none -Oz -o "$output_vcf" "$input_vcf"
    echo "Cleaned VCF saved as: $output_vcf"
  done
}
...
run_beagle() {
  ...
  reference_vcf="${cleaned_reference_dir}/ALL.chr${chr}.cleaned.vcf.gz"
