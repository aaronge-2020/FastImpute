...
async function loadAllModels() {
    const modelFolder = "./Data/model_results_unphased_all_PRS/linear_regression/onnx_models";
    const modelPromises = [];
  
    for (let i = 1; i <= 22; i++) {
      const modelPath = `${modelFolder}/final_model_chr${i}.onnx`;
      modelPromises.push(fetch(modelPath).then(response => response.arrayBuffer()).then(buffer => ort.InferenceSession.create(buffer)));
    }
  ...
  async function makePredictions(chrNumber, inputData) {
    if (!document.models || !document.models[`chr${chrNumber}`]) {
      alert(`Please load all models and ensure the model for chromosome ${chrNumber} is available.`);
      return;
    }
    ...
    alert(`Predictions made for chromosome ${chrNumber}. Check the console for details.`);
    return output;
